package com.eurekanetwork.wallet.ui.fragment.qr_code_recognition_fragment;
...
public class QrCodeRecognitionFragment extends Fragment implements ZXingScannerView.ResultHandler {
    ...
    @BindView(com.eurekanetwork.wallet.R.id.camera_container)
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(com.eurekanetwork.wallet.R.layout.fragment_qrcode_camera, container, false);
    }
    ...
    public void handleResult(Result result) {
        ...
        Pattern pattern = Pattern.compile("eurekacoin:(.*?)\\?");
        Matcher matcher = pattern.matcher(result.getText());
        if (matcher.find()) {
            receiveAddr = matcher.group(1);
        } else {
            pattern = Pattern.compile("eurekacoin:(.*?)$");
