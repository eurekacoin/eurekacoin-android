...
import static com.eurekanetwork.wallet.utils.crypto.KeyStoreHelper.trimEndSpaces;
...
class PinInteractorImpl implements PinInteractor {
    ...
    private String getFourDigitPassword() {
        String encryptedPinHash = EurekacoinSharedPreference.getInstance().getPassword(mContext);
        return KeyStoreHelper.decrypt(EUREKACOIN_PIN_ALIAS, encryptedPinHash);
    }

    @Override
    public String getSixDigitPassword() {
        return EurekacoinSharedPreference.getInstance().getSixDigitPassword(mContext);
    }

    @Override
    public Integer getFailedAttemptsCount() {
        return EurekacoinSharedPreference.getInstance().getFailedAttemptsCount(mContext);
    }

    @Override
    public Long getBanTime() {
        return EurekacoinSharedPreference.getInstance().getBanTime(mContext);
    }

    @Override
    public void setFailedAttemptsCount(int failedAttemptsCount) {
        EurekacoinSharedPreference.getInstance().setFailedAttemptsCount(mContext, failedAttemptsCount);
    }

    @Override
    public void setBanTime(long banTime) {
        EurekacoinSharedPreference.getInstance().setBanTime(mContext, banTime);
    }

    @Override
    public void saveSixDigitPassword(String password) {
        EurekacoinSharedPreference.getInstance().saveSixDigitPassword(mContext, password);
    }
    ...
    public void savePassphraseSaltWithPin(String pin, String passphrase) {
        ...
        EurekacoinSharedPreference.getInstance().saveSeed(mContext, encryptedSaltPassphrase);
    }

    @Override
    public byte[] getSaltPassphrase() {
        String encryptedSaltPassphrase = EurekacoinSharedPreference.getInstance().getSeed(mContext);
        return Base64.decode(encryptedSaltPassphrase, Base64.DEFAULT);
    }

    @Override
    public String getTouchIdPassword() {
        return EurekacoinSharedPreference.getInstance().getTouchIdPassword(mContext);
    }

    @Override
    public void saveTouchIdPassword(String password) {
        EurekacoinSharedPreference.getInstance().saveTouchIdPassword(mContext, password);
    }
    ...
    public void setKeyGeneratedInstance(boolean isKeyGenerated) {
        EurekacoinSharedPreference.getInstance().setKeyGeneratedInstance(mContext, isKeyGenerated);
    }
