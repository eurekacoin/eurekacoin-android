...
import com.eurekanetwork.wallet.ui.activity.splash_activity.SplashActivity;
import com.eurekanetwork.wallet.ui.base.base_activity.BaseActivity;
...
import com.eurekanetwork.wallet.ui.fragment.wallet_fragment.WalletFragment;
import com.eurekanetwork.wallet.ui.fragment.wallet_main_fragment.WalletMainFragment;
import com.eurekanetwork.wallet.utils.CustomContextWrapper;
...
import static com.eurekanetwork.wallet.ui.base.base_fragment.BaseFragment.BACK_STACK_ROOT_TAG;
...
public class MainActivity extends BaseActivity implements MainActivityView {
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (intent != null && intent.getAction() != null) {
            switch (intent.getAction()) {
                case EurekacoinIntent.OPEN_FROM_NOTIFICATION:
                    openRootWallet();
                    break;
                default:
                    break;
    ...
    public boolean checkTouchIdEnable() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (EurekacoinSharedPreference.getInstance().isTouchIdEnable(getContext())) {
    ...
    public void initializeViews() {
        ...
        switch (intent.getAction()) {
            case EurekacoinIntent.SEND_FROM_SDK:
                getPresenter().setSendFromIntent(true);
                mAddressForSendAction = intent.getStringExtra(EurekacoinIntent.SEND_ADDRESS);
                mAmountForSendAction = intent.getStringExtra(EurekacoinIntent.SEND_AMOUNT);
                mTokenAddressForSendAction = intent.getStringExtra(EurekacoinIntent.SEND_TOKEN);
                break;
    ...
    protected void attachBaseContext(Context newBase) {
        super.attachBaseContext(CustomContextWrapper.wrap(newBase, EurekacoinSettingSharedPreference.getInstance().getLanguage(newBase)));
    }

    public EurekacoinApplication getEurekacoinApplication() {
        return (EurekacoinApplication) getApplication();
    }
    ...
    public Realm getRealm(){
        return ((EurekacoinApplication)getApplication()).getRealm();
    }
}