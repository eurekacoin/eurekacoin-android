package com.eurekanetwork.wallet.utils.migration_manager;
...
import static com.eurekanetwork.wallet.utils.crypto.KeyStoreHelper.EUREKACOIN_PIN_ALIAS;
...
public class MigrationManager {
    ...
    private static KeystoreMigrationResult migratePassphrase(Context context) {
        if (TextUtils.isEmpty(EurekacoinSharedPreference.getInstance().getKeystoreMigrationState(context))) {
            String encryptedSaltPassphrase = EurekacoinSharedPreference.getInstance().getSeed(context);
            if (!TextUtils.isEmpty(encryptedSaltPassphrase)) {
                try {
                    KeyStoreHelper.createKeys(context, EUREKACOIN_PIN_ALIAS);
                    byte[] bytes = KeyStoreHelper.decryptToBytes(EUREKACOIN_PIN_ALIAS, encryptedSaltPassphrase);
                    savePassphraseSalt(context, bytes);
                    EurekacoinSharedPreference.getInstance().setKeyStoreMigrationState(context, KeystoreMigrationResult.SUCCESS.name());
                    return KeystoreMigrationResult.SUCCESS;
                } catch (NoSuchProviderException | NoSuchAlgorithmException | InvalidAlgorithmParameterException | RuntimeException e) {
                    return KeystoreMigrationResult.ERROR;
                }
            } else {
                EurekacoinSharedPreference.getInstance().setKeyStoreMigrationState(context, KeystoreMigrationResult.NOT_NEED.name());
                return KeystoreMigrationResult.NOT_NEED;
            }
        } else {
            return KeystoreMigrationResult.NOT_NEED;
    ...
    private static void migratePinCode(Context context) {
        boolean isSixDigit;
        String encryptedPinHash = EurekacoinSharedPreference.getInstance().getSixDigitPassword(context);
        isSixDigit = !TextUtils.isEmpty(encryptedPinHash);
        if (!isSixDigit) {
            encryptedPinHash = EurekacoinSharedPreference.getInstance().getPassword(context);
            if (TextUtils.isEmpty(encryptedPinHash)) {
                return;
            }
        }
        String decrypt = KeyStoreHelper.decrypt(EUREKACOIN_PIN_ALIAS, encryptedPinHash);
        if (isSixDigit) {
            EurekacoinSharedPreference.getInstance().saveSixDigitPassword(context, decrypt);
        } else {
            EurekacoinSharedPreference.getInstance().savePassword(context, decrypt);
        }
    }
    ...
    private static void savePassphraseSalt(Context context, byte[] saltPassphrase) {
        ...
        EurekacoinSharedPreference.getInstance().saveSeed(context, base64);
    }

    private void clearWallet(Context context, Realm realm) {
        EurekacoinSharedPreference.getInstance().forceClear(context);
